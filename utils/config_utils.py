...
def generate_peft_config(train_config, kwargs):
    ...
    ConfigCls = configs[names.index(train_config.peft_method)]

    # 关键：实例化（避免对“类”取字段默认值）
    config_obj = ConfigCls() if is_dataclass(ConfigCls) else ConfigCls

    # 更新配置（对实例更新）
    update_config(config_obj, **kwargs)

    # 取字段值（对实例取）
    params = {f.name: getattr(config_obj, f.name) for f in fields(config_obj)}
    peft_config = peft_configs[names.index(train_config.peft_method)](**params)
    return peft_config
...
def generate_dataset_config(train_config, kwargs):
    ...
    dataset_paths = {
        ...
        "unsafe_dataset": f"{base_path}/pure_bad_dataset/dataset/unsafe.json",
        "metamath_dataset": f"{base_path}/metamath_dataset/dataset/metamath_{mode}.json",
    }
